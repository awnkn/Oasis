          ...
          {FAQ_ITEMS.map((item, i) => (
            <details key={i} className="group px-6 py-5 [&_summary::-webkit-details-marker]:hidden" open={i === 0}>
              <summary className="flex cursor-pointer list-none items-center justify-between gap-4 font-display text-lg font-semibold text-oasis-950">
                {item.q}
                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-blue-50 text-blue-600 transition group-open:rotate-45">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden>
                    <path d="M12 5v14M5 12h14" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
                  </svg>
                </span>
              </summary>
              <p className="mt-3 text-oasis-900/70 leading-relaxed">{item.a}</p>
            </details>
          ))}
        </div>

        {/* Quick facts + CTA */}
        <div className="mt-10 rounded-3xl bg-blue-50/60 p-8 text-center ring-1 ring-blue-100">
          <p className="font-display text-2xl font-semibold text-oasis-950">
            Ready for a slow, sunlit day?
          </p>
          <p className="mt-2 text-oasis-900/70">
